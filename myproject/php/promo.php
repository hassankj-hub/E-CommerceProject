...
<head>
  ...
  <title>SOCCERSHOES XI - Active Promos</title>
...
<body class="bg-slate-50 text-slate-800 min-h-screen flex">
  ...
  <aside class="w-72 bg-[#00C853] text-white flex flex-col justify-between p-6 shrink-0 rounded-r-[2rem] shadow-xl">
    <div>
        ...
        <div class="flex items-center gap-3">
          <div class="relative">
            <div class="w-12 h-12 rounded-full border-2 border-white bg-white overflow-hidden flex items-center justify-center shadow-sm">
              <!-- Corrected stepout file path to restore sidebar logo imagery rendering -->
      ...
      <nav class="space-y-1.5">
        ...
        <a href="../php/promo.php" class="flex items-center gap-4 px-5 py-3.5 bg-white text-[#00C853] rounded-2xl shadow-md font-bold text-xs tracking-wide relative">
  ...
  <main class="flex-1 p-8 overflow-y-auto max-w-[1400px] mx-auto w-full">
    <header class="flex justify-between items-start mb-8">
      <div>
        <div class="flex items-center gap-2">
          ...
          <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Voucher & Campaign Vectors</h1>
        </div>
        <p class="text-[11px] text-[#00C853] font-bold tracking-wide mt-0.5 ml-3.5">PROMOTIONAL HOOK ENGINE</p>
      </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm max-w-md">
      <div class="flex items-start justify-between border-b border-slate-100 pb-4 mb-4">
        <div>
          <h4 class="text-xs font-extrabold text-slate-900 uppercase">Pitch Control Campaign</h4>
          <p class="text-[10px] text-slate-400 font-medium mt-0.5">Targeting high-tier professional players</p>
        </div>
        <span class="bg-emerald-50 text-[#00C853] text-[9px] font-bold px-2 py-0.5 rounded">LIVE</span>
      </div>
      <div class="bg-slate-50 rounded-xl p-4 border border-dashed border-slate-200 flex justify-between items-center">
        <div>
          <span class="text-[9px] text-slate-400 font-bold block uppercase tracking-wider">Campaign Passcode</span>
          <span id="promo-code" class="text-xs font-extrabold text-slate-900 tracking-wider">ELITEBOOTSXI</span>
        </div>
        <button onclick="copyPromoCode()" class="bg-[#00C853] text-white text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-sm hover:bg-emerald-600 transition">Copy Key</button>
      </div>
    </div>
  </main>
  ...
  function copyPromoCode() {
      ...
      navigator.clipboard.writeText(codeText).then(() => {
          alert('Promo key copied to clipboard matrix: ' + codeText);
      }).catch(err => {
          console.error('Failed to capture copy sequence:', err);
      });
  }
  </script>
</body>
