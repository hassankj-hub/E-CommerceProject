...
// 3. Extract dynamic session variables with safe default fallbacks aligned with BootsXI credentials
...
<head>
  ...
  <title>SOCCERSHOES XI - User Account Profiles</title>
  ...
  <aside class="w-72 bg-[#00C853] text-white flex flex-col justify-between p-6 shrink-0 rounded-r-[2rem] shadow-xl">
    <div>
      ...
      <nav class="space-y-1.5">
        ...
        <a href="../php/user.php" class="flex items-center gap-4 px-5 py-3.5 bg-white text-[#00C853] rounded-2xl shadow-md font-bold text-xs tracking-wide relative">
  ...
  <main class="flex-1 p-8 overflow-y-auto max-w-[1400px] mx-auto w-full">
    <header class="flex justify-between items-start mb-8">
      <div>
        <div class="flex items-center gap-2">
          ...
          <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">User Account Matrix</h1>
        </div>
        <p class="text-[11px] text-[#00C853] font-bold tracking-wide mt-0.5 ml-3.5">ADMINISTRATIVE AUTHORIZATION STATUS</p>
      </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm max-w-xl">
      <div class="flex items-center gap-4 mb-6">
        <div class="w-20 h-20 rounded-full border-2 border-[#00C853] bg-slate-50 overflow-hidden flex items-center justify-center shadow-inner">
          <img src="../images/logo.png?v=1" alt="Profile Vector" class="w-[70%] h-[70%] object-contain" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
          <div class="hidden w-full h-full bg-emerald-500 text-white flex items-center justify-center font-bold text-xl uppercase">
            <?php echo substr(htmlspecialchars($user_name), 0, 2); ?>
          </div>
        </div>
        <div>
          ...
          <p class="text-xs text-[#00C853] font-bold">Senior Platform Auditor</p>
          <p class="text-[10px] text-slate-400 font-medium mt-1">Authorized Node Session Token: <span class="text-emerald-500 font-bold">Active</span></p>
        </div>
      </div>
      <div class="border-t border-slate-100 pt-4 space-y-2 text-xs font-semibold text-slate-600">
        <div class="flex justify-between py-2 border-b border-slate-50">
          <span>Associated Terminal Mail:</span>
          <span class="text-slate-900 font-bold"><?php echo htmlspecialchars($user_email); ?></span>
        </div>
        <div class="flex justify-between py-2">
          <span>System Access Clearance Vector:</span>
          <span class="text-slate-900 font-bold">Level 4 Root Admin</span>
        </div>
      </div>
    </div> 
  </main>
